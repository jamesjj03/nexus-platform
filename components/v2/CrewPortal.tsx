...
import { CheckCircle2, ClipboardList, Hand, LayoutDashboard, LogOut, Package, Search, Send, Truck, UploadCloud, Wrench } from "lucide-react";
...
type RequestItem = { id: string; kind: string; item: string; assetIds?: string[]; requestedBy: string; crew: string; status: "Pending" | "Approved" | "Denied" | "Completed"; note: string; startDate?: string; endDate?: string; createdAt: string };
...
function normalizeBoard(data: NexusBoardData, slug: string): Required<NexusBoardData> { return { jobs: data.jobs || [], equipment: data.equipment || [], tools: data.tools || [], inventory: data.inventory || [], issues: data.issues || [], staff: data.staff || [], requests: data.requests || [], checkouts: data.checkouts || [], updatedAt: data.updatedAt || "", companySlug: slug }; }
...
export function CrewPortal({ companySlug }: { companySlug?: string }) {
  ...
  async function handleFile(file?: File) { if (!file) return; setStatus("Uploading..."); const result = await uploadCompanyFile(activeCompany.slug, file); const url = result.ok ? result.url : URL.createObjectURL(file); setUploads((list) => [{ id: String(Date.now()), name: file.name, url, kind, note }, ...list]); setStatus(result.ok ? "Uploaded live." : `${result.message} Local preview is showing.`); if (result.ok) setNote(""); }
  ...
  async function submitRequest(mode: "today" | "multi" = "multi", singleAssetId?: string) {
    ...
    const next: RequestItem = { id: `req-${Date.now()}`, kind: "Checkout", item: names || "General checkout request", assetIds: ids, requestedBy: requestBy, crew: requestCrew, status: "Pending", note: requestNote || (mode === "today" ? "Requesting checkout for today." : "Requesting checkout extension."), startDate: mode === "today" ? today() : requestStart, endDate: mode === "today" ? today() : requestEnd, createdAt: new Date().toISOString() };
    ...
    setSelectedAssets([]); setRequestNote(""); setTab("request");
  }
  ...
  return <main className="ff-light-page"><div className="ff-crew-shell">
    ...
    <div className="ff-crew-tabs-v2 ff-crew-tabs-five"><button className={tab === "today" ? "active" : ""} onClick={() => setTab("today")}><ClipboardList /> Jobs</button><button className={tab === "equipment" ? "active" : ""} onClick={() => setTab("equipment")}><Truck /> Equipment</button><button className={tab === "tools" ? "active" : ""} onClick={() => setTab("tools")}><Wrench /> Tools</button><button className={tab === "inventory" ? "active" : ""} onClick={() => setTab("inventory")}><Package /> Inventory</button><button className={tab === "request" ? "active" : ""} onClick={() => setTab("request")}><Send /> Requests</button><button className={tab === "upload" ? "active" : ""} onClick={() => setTab("upload")}><UploadCloud /> Upload</button></div>
